# ============================================================================
# AVD Deployment - EntraDS (Azure AD Domain Services) Identity Provider
# ============================================================================
# Use case: Managed Active Directory in Azure (no on-premises infrastructure)
# Authentication: Azure AD Domain Services (Microsoft Entra Domain Services)
# Domain join: ENABLED (VMs join to Azure AD DS managed domain)
# Requirements: Azure AD DS deployed and configured, VNet peering configured
# ============================================================================

deployment_prefix             = "ETF1"
deployment_environment        = "Dev"

# ============================================================================
# IDENTITY CONFIGURATION - EntraDS (Azure AD Domain Services)
# ============================================================================
avd_identity_service_provider = "EntraDS"

# Domain settings: REQUIRED - Replace with your Azure AD DS domain details
identity_domain_name           = "contoso.onmicrosoft.com"                        # CHANGE: Your Azure AD DS domain
avd_domain_join_user_name      = "adadmin@contoso.onmicrosoft.com"                # CHANGE: Azure AD DS admin account
avd_domain_join_user_password  = "YourSecurePassword123!"                         # CHANGE: Azure AD DS admin password
avd_ou_path                    = "OU=AADDC Computers,DC=contoso,DC=onmicrosoft,DC=com"  # OPTIONAL: Target OU (default AADDC Computers)

# ============================================================================
# LOCATION SETTINGS
# ============================================================================
avd_session_host_location     = "East US"
avd_management_plane_location = "East US"
management_plane_location_short = "eus"
session_host_location_short     = "eus"
scaling_plan_time_zone          = "UTC"

# ============================================================================
# SESSION HOST CREDENTIALS
# ============================================================================
avd_vm_local_user_name        = "avdadmin"
avd_vm_local_user_password    = "ChangeMe!"

# ============================================================================
# HOST POOL CONFIGURATION
# ============================================================================
avd_host_pool_type            = "Pooled"
host_pool_preferred_app_group_type = "Desktop"
avd_host_pool_load_balancer_type   = "DepthFirst"

# ============================================================================
# SESSION HOST DEPLOYMENT
# ============================================================================
avd_deploy_session_hosts      = true
avd_deploy_session_hosts_count = 1
avd_session_host_count_index  = 1
avd_session_hosts_size        = "Standard_D2als_v6"

# ============================================================================
# NETWORKING (Using Terraform-created VNET)
# ============================================================================
# IMPORTANT: For EntraDS, ensure VNet is peered with Azure AD DS VNet
# and DNS servers point to Azure AD DS domain controllers (typically 10.0.0.4, 10.0.0.5)
existing_vnet_avd_subnet_resource_id = ""
existing_vnet_private_endpoint_subnet_resource_id = ""
vnet_resource_group           = "rg-network"
vnet_name                     = "vnet-avd"
subnet_name                   = "snet-avd"

# ============================================================================
# STORAGE CONFIGURATION
# ============================================================================
create_avd_fslogix_deployment = true
create_app_attach_deployment  = false

# ============================================================================
# AZURE SUBSCRIPTION & TENANT
# ============================================================================
subscription_id = "bd67233d-38af-45db-a98e-a10a2b443d8d"
tenant_id       = "d3edc3c2-c1ad-469e-a53a-7dac46272775"

# ============================================================================
# TAGS
# ============================================================================
create_resource_tags          = false

tags = {
  Environment = "Dev"
  IdentityProvider = "EntraDS"
}
